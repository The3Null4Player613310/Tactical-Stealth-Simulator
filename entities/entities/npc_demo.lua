...
function ENT:Initialize()
	...
	if (math.random(0,1) == 1) then
		if SERVER then self:SetModel(PMMaleCitizen[math.random(#PMMaleCitizen)]) end--set random player model from array
		--ply:EmitSound(VMale[math.random(#VMale)],75,100,1,CHAN_AUTO) -- use V as prefix
	else
		if SERVER then self:SetModel(PMFemaleCitizen[math.random(#PMFemaleCitizen)]) end--set random player model from array
		--ply:EmitSound(VMale[math.random(#VMale)],75,100,1,CHAN_AUTO) -- use V as prefix
	end

    self.WalkerColor = Vector((math.random(101)-1)/200,(math.random(101)-1)/200,(math.random(101)-1)/200)
...
function ENT:OnNavAreaChanged( old, new)
    /*if new:HasAttributes( NAV_MESH_JUMP ) then --WIP
        self:Jump()
    end*/
