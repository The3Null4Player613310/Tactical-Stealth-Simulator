...
function EFFECT:Think()
	...
	if (CurTime()>self.DieTime) or (GetGlobalInt( "GlobalWeather", 0 ) != 1) or !(util.IsSkyboxVisibleFromPoint(LocalPlayer():EyePos())) then 
		self.Emitter:Finish()
		return false 
	end
	local speed = 192 --128
	...
	for i=1, self.Particles do
		...
		if (spawnpos != Vector(0,0,0)) then
			...
			local particle = emitter:Add("particle/water/waterdrop_001a", spawnpos)--effects/rain_cloud
			if (particle and tr.Hit) then
				...
				particle:SetEndSize(1.5)--1
				particle:SetStartLength(0)
				particle:SetEndLength(16)
				particle:SetAirResistance(0)
				particle:SetLighting( true )
				particle:SetCollide(true)--true
